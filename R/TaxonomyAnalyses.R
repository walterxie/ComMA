...
plotTaxonomy <- function(all.counts.sums, taxa.ref="", taxa.rank="phylum", group.rank="kingdom", 
  ...
  require(ggplot2)
  p <- ggplot(z) + 
    geom_point(data = z[variable == "count.1"], 
               aes(x = taxa, y = value, colour = group), shape = 1, size = 2) +
    geom_point(data = z[variable == "sum.1"], 
               aes(x = taxa, y = value, colour = group), shape = 5, size = 2, show.legend = NA) +
    geom_point(data = z[variable == "count.2"], 
               aes(x = taxa, y = value, colour = group), shape = 16, size = 2) +
    #geom_point(data = z[variable == "sums.2"], aes(x = taxa, y = value, colour = group), shape = 17, size = 2) +
    geom_line(data = z, 
              aes(x = taxa, y = value, group=interaction(taxa, gene), colour = group), 
              size = 0.5, alpha = 0.5) + 
...
sumReadsOTUs <- function(cm.taxa.list, taxa.ref="", taxa.rank="phylum", group.rank="kingdom", 
  ...
  if (!is.na(fig.folder)) {
    p <- ComMA::plotTaxonomy(all.counts.sums)
